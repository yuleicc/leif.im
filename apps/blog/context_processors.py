...
def theme(request):
    ...
    theme_data['use_motion'] = "use-motion" if getattr(
        settings, "USE_MOTION") else ""
    theme_data['google_site_verification'] = getattr(
        settings, "GOOGLE_SITE_VERIFICATION", "")
    theme_data['baidu_site_verification'] = getattr(
        settings, "BAIDU_SITE_VERIFICATION", "")
    theme_data['qihu_site_verification'] = getattr(
        settings, "QIHU_SITE_VERIFICATION", "")
    ...
    theme_data['duoshuo_shortname'] = getattr(
        settings, "DUOSHUO_SHORTNAME", '')
    ...
    theme_data['google_analytics'] = getattr(
